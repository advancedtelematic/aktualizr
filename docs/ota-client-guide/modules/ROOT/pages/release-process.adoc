...
There is the gitlab CI job that will automatically update the recipe, build a bottle, and upload the bottle to the github release page when a new release is created. The one thing it doesn't do is make the pull request to update the recipe in the tap.

You will find the new recipe attached to the gitlab CI job named `build-osx-release` that should be triggered in the scope of the latest https://main.gitlab.in.here.com/olp/edge/ota/connect/client/aktualizr/pipelines?scope=tags&page=1[release CI pipeline]. You can download the file from there, and submit the changes as a pull request to the https://github.com/advancedtelematic/homebrew-otaconnect/[homebrew-otaconnect repo].
