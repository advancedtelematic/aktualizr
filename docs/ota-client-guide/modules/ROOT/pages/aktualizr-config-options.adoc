...
| `curl_bandwith`        |    0                  | Use it to limit curl download speed (counted in bytes per second). Aktualizr use this value for https://curl.se/libcurl/c/CURLOPT_MAX_SEND_SPEED_LARGE.html[CURLOPT_MAX_RECV_SPEED_LARGE] needs curl 7.41.0+. Default 0 removes all limitations. This options has no effect when downloading  OSTREE update.
