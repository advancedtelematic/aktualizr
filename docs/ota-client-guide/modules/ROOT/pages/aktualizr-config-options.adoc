...
| `curl_bandwith`        |    0                  | Use it to limit curl download speed. Aktualizr use this value for https://curl.se/libcurl/c/CURLOPT_MAX_SEND_SPEED_LARGE.html[CURLOPT_MAX_RECV_SPEED_LARGE] needs curl 7.41.0+. Default 0 removes all limitations. This options has no effect when downloading  OSTREE update.
| `use_oscp`             | false                 | Verify the certificate's status. If enabled it initialize default libCURL based http client with https://curl.se/libcurl/c/CURLOPT_SSL_VERIFYSTATUS.html[CURLOPT_SSL_VERIFYSTATUS]. It is dessable by default. But it is possible define macro USE_OSCP to enable it by default. This options has no effect when downloading  OSTREE update.
|==========================================================================================
