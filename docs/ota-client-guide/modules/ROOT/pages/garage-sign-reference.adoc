...
+++<details><summary>+++
`root [pull|push|get-unsigned|increment-version|key|sign]`: Manages root-of-trust metadata for a repository.
+++</summary><div>+++
...
+++<details><summary>+++
`targets [init|add|add-uploaded|delete|sign|pull|push|increment-version|get-unsigned|upload|delegations]`: (Only for repositories of the `reposerver` type) Manages Targets metadata.
// tag::target-term[]
