...
| *OSTree* |  OSTree is a tool for managing Linux filesystems, and delivers atomic updates with rollback capability and built-in delta support. OSTree has several advantages over traditional dual-bank systems, but the most important one is that it conserves your network bandwidth and minimizes your data storage footprint. It does this by sharing files with the same contents across file system deployments.
