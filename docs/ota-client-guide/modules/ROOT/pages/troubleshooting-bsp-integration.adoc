...
For example, let's say for target_board_example, the environment setup script added a line to the `local.conf` that overrode the INITRAMFS_IMAGE value. To
fix this, you need to specify the correct value in your `local.conf` when you build:
[source,bash]
----
