...
| `secondary_config_file`   | `""`         | A path to a json file containing configuration of Secondary ECU(s) to be registered with the primary. link:{aktualizr-github-url}/config/posix_secondary_config.json[Example is here]. It's a new way to configure secondaries and is replacement of `secondary_configs_dir`. The later is going to be removed once the virtual secondary implementation is fully ported to the new approach.
