...
Aktualizr is configured via `.toml` config file and passed to the application via the `--config` flag. All fields are optional, and many have reasonable defaults that should be used unless you have a particular need to do otherwise.
...
| `ipdiscovery_host`         | `"::ffff:127.0.0.1"`                   | Subnetmask for primary to broadcast for secondary discovery.
...
|==========================================================================================
| Name                      | Default      | Description
| `polling`                 | `true`       | Enable continuous polling of the server. (The inverse of `--poll-once`.)
...
| `key_type`                | `"kRSA2048"` | Type of cryptographic keys to use. Options: `"ED25519"`, `"RSA2048"`, `"kRSA4096"`.
...
| `schemas_path`            | `"/usr/lib/sota/schemas"` | Path to the directory containing schema migrations. Only used with `sqlite`.
|==========================================================================================
