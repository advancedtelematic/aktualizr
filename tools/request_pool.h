...
class RequestPool {
  public:
    ...
    void add_upload(OSTreeObject::ptr request) { upload_queue_.push_back(request); }
    void abort() { query_queue_.clear(); upload_queue_.clear(); };
    bool is_idle() { return query_queue_.empty() && upload_queue_.empty() && running_requests_ == 0; }
    void loop(); // one iteration of request-listen loop, launches up to max_requests_ requests, then listens for the result
