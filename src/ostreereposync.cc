...
bool ArchiveModeRepo(const fs::path& repo_dir) {
  ...
  return (open_succeed && (ostree_repo_get_mode(repo) == OSTREE_REPO_MODE_ARCHIVE));
}
...
bool LocalPullRepo(const fs::path& src_repo_dir, const fs::path& dst_repo_dir) {
  ...
  if (!ostree_repo_create(dst_repo, OSTREE_REPO_MODE_ARCHIVE, NULL, &error)) {
  ...
  if (!ostree_repo_pull_with_options(dst_repo, src_repo_url.c_str(),
