...
Histogram::Histogram() {
  hdr_init(1, 3L * 60 * 1000, 3, &histogram);
};
...
int64_t Histogram::totalCount() {
  return histogram->total_count;
}

void Statistics::recordSuccess(const std::chrono::milliseconds& duration) {
  successDurations.record(duration);
}

void Statistics::recordFailure(const std::chrono::milliseconds& duration) {
  errorDurations.record(duration);
}
