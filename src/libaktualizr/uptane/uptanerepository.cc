...
void RepositoryCommon::initRoot(RepositoryType repo_type, const std::string& root_raw) {
  try {
    ...
    throw Uptane::Exception(repo_type.toString(), "Failed to initialize root");
  }
}
...
void RepositoryCommon::verifyRoot(const std::string& root_raw) {
  try {
    ...
    throw Uptane::SecurityException(type.toString(), "Signature verification for Root metadata failed");
  }
}
...
void RepositoryCommon::updateRoot(INvStorage& storage, const IMetadataFetcher& fetcher,
  ...
  if (rootExpired()) {
    throw Uptane::ExpiredMetadata(repo_type.toString(), "Expired root");
  }
}
