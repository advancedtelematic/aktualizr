...
TEST(command, SendUpdateReport_comand_to_json) {
  ...
  operation_result.result_code = data::UpdateResultCode::kNotFound;
  ...
  EXPECT_EQ(json["fields"][0]["operation_results"][0]["result_code"].asUInt(),
            static_cast<int>(data::UpdateResultCode::kNotFound));
...
TEST(command, SendUpdateReport_comand_from_json) {
  ...
  EXPECT_EQ(comand.update_report.operation_results[0].result_code, data::UpdateResultCode::kNotFound);
