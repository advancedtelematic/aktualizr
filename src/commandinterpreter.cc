...
CommandInterpreter::CommandInterpreter(SotaClient *sota_in,
...
void CommandInterpreter::interpret() {
  ...
  while (*commands_channel >> command) {
    ...
    if (command->variant == "GetUpdateRequests") {
      ...
      command::StartDownload *start_download_command =
          command->toChild<command::StartDownload>();
      sota->startDownload(start_download_command->update_request_id);
    } else if (command->variant == "SendUpdateReport") {
      command::SendUpdateReport *send_update_report_command =
          command->toChild<command::SendUpdateReport>();
