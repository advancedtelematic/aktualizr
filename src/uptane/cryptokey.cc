...
OnDiskCryptoKey::OnDiskCryptoKey(const boost::shared_ptr<INvStorage> &backend, const Config &config)
    : backend_(backend),
      config_(config)
#ifdef BUILD_P11
      ,
...
std::string OnDiskCryptoKey::getPkey() const {
  std::string pkey;
#ifdef BUILD_P11
  ...
  if (config_.tls.pkey_source == kFile) {
    ...
    throw std::runtime_error("tls.pkey_source is pkcs11, but aktualizr is not builded with pkcs11 support");
  }
  return pkey;
}

std::string OnDiskCryptoKey::getCert() const {
  std::string cert;
#ifdef BUILD_P11
  ...
  if (config_.tls.cert_source == kFile) {
    ...
    throw std::runtime_error("tls.cert_source is pkcs11, but aktualizr is not builded with pkcs11 support");
  }
  return cert;
}

std::string OnDiskCryptoKey::getCa() const {
  std::string ca;
#ifdef BUILD_P11
  ...
  if (config_.tls.ca_source == kFile) {
    ...
    throw std::runtime_error("tls.ca_source is pkcs11, but aktualizr is not builded with pkcs11 support");
  }
  return ca;
}

std::string OnDiskCryptoKey::getCN() const {
...
void OnDiskCryptoKey::copyCertsToCurl(HttpInterface *http) {
...
Json::Value OnDiskCryptoKey::signTuf(const Json::Value &in_data) {
  ENGINE *crypto_engine = NULL;
  std::string private_key;
#ifdef BUILD_P11
  if (config_.uptane.key_source == kPkcs11) crypto_engine = p11.getEngine();
#endif
...
std::string OnDiskCryptoKey::getUptanePublicKey() {
...
std::string OnDiskCryptoKey::generateUptaneKeyPair() {
  std::string primary_public;
  ...
  if (config_.uptane.key_source == kFile) {
    ...
    // dummy read to check if the key is present
    if (!p11.readUptanePublicKey(&primary_public)) {
      p11.generateUptaneKeyPair();
    }
    // realy read the key
    if (primary_public.empty() && !p11.readUptanePublicKey(&primary_public)) {
      throw std::runtime_error("Could not get uptane keys");
    }
    return primary_public;
#else
