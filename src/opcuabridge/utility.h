...
template <typename A, typename T> struct serialize_field {};

#define SERIALIZE_FIELD_DECL_BEGIN(STREAM)                                     \
  template <typename T> struct serialize_field<STREAM, T> {                    \
...
SERIALIZE_FIELD_DECL_BEGIN(DATA_SERIALIZATION_IN_XML_STREAM) {
  ar &boost::serialization::make_nvp(xml_tag.c_str(), d);
}
SERIALIZE_FIELD_DECL_END
