...
FileSetEntryHash::result_type
FileSetEntryHash::operator()(FileSetEntryHash::argument_type const& e) const {
  result_type seed = 0;
  argument_type  x = e;
  for(; *x != '\0'; ++x) {
...
std::size_t UpdateFileList(FileList* filelist, const fs::path& repo_dir_path) {
  ...
  fs::recursive_directory_iterator
    repo_dir_it_end, repo_dir_it(repo_dir_path);
  for (; repo_dir_it != repo_dir_it_end; ++repo_dir_it) {
    ...
    if (fs::is_regular_file(ent_path)) {
      ...
      std::copy(rel_path.native().begin(),
                rel_path.native().end(),
