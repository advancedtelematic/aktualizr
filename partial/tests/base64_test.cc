...
TEST(tinybase64, decode) {
  ...
  std::string mod0 = "123456789";
  EXPECT_EQ(base64_decode(Utils::toBase64(mod0).c_str(), Utils::toBase64(mod0).length(), decoded_buf) , 9);
  EXPECT_EQ(std::string(reinterpret_cast<char*>(decoded_buf), 9), mod0);

  std::string mod2 = "12345678";
  EXPECT_EQ(base64_decode(Utils::toBase64(mod2).c_str(), Utils::toBase64(mod2).length(), decoded_buf) , 8);
  EXPECT_EQ(std::string(reinterpret_cast<char*>(decoded_buf), 8), mod2);

  std::string mod1 = "1234567";
  EXPECT_EQ(base64_decode(Utils::toBase64(mod1).c_str(), Utils::toBase64(mod1).length(), decoded_buf) , 7);
  EXPECT_EQ(std::string(reinterpret_cast<char*>(decoded_buf), 7), mod1);

  std::string len1 = "1";
  EXPECT_EQ(base64_decode(Utils::toBase64(len1).c_str(), Utils::toBase64(len1).length(), decoded_buf) , 1);
  EXPECT_EQ(std::string(reinterpret_cast<char*>(decoded_buf), 1), len1);

  std::string len2 = "12";
  EXPECT_EQ(base64_decode(Utils::toBase64(len2).c_str(), Utils::toBase64(len2).length(), decoded_buf) , 2);
  EXPECT_EQ(std::string(reinterpret_cast<char*>(decoded_buf), 2), len2);

  std::string len0 = "";
  EXPECT_EQ(base64_decode(Utils::toBase64(len0).c_str(), Utils::toBase64(len0).length(), decoded_buf), 0);
}
